//AdminClaims.jsx - Admin interface for managing pending claims and disputes.
//  Allows admins to view all pending claims and take action (approve/reject) on them.
//  This page is crucial for maintaining the integrity of the system by ensuring that disputes are resolved fairly and efficiently.
//  It also serves as a central hub for admins to monitor the status of claims and intervene when necessary.
...
export default function AdminClaims() {
  ...
  const handleUpdateStatus = async (claimId, newStatus) => {
    try {
      ...
      await axios.put(
        `/claims/${claimId}/status?status=${newStatus}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  if (loading) {
    return (
      <div className="p-8 w-full bg-gray-50 flex items-center justify-center">
