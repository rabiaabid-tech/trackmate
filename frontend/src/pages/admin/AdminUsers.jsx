// AdminUsers.jsx - Admin interface for managing user accounts and their statuses.
//  This page allows admins to view all users, toggle their blocking status, and access detailed user histories.
...
export default function AdminUsers() {
  ...
  const handleViewHistory = async (user) => {
    ...
    try {
      ...
      const [lostRes, foundRes] = await Promise.all([
        axios.get("/lost/admin/all", config),
        axios.get("/found/admin/all", config),
      ]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20 w-full bg-gray-50">
      ...
      <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-gray-100">
        <table className="w-full text-left">
          <thead className="bg-[#0B1F4D] text-white">
            <tr>
              <th className="p-6 text-[10px] font-black uppercase tracking-widest w-16">
                ID
              </th>
              <th className="p-6 text-[10px] font-black uppercase tracking-widest text-left">
                Student
              </th>
              <th className="p-6 text-[10px] font-black uppercase tracking-widest text-left">
                Email
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((u) => (
                ...
                >
                  <td className="p-6 font-mono text-sm text-gray-500 font-bold">
                    #{u.id}
                  </td>
                  ...
                  <td className="p-6 font-bold text-[#0B1F4D]">
                    <div className="flex items-center gap-2">
                      ...
                      {u.is_blocked && (
                        <span className="inline-block text-[8px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full uppercase tracking-widest">
                          Blocked
                        </span>
                      )}
                    </div>
