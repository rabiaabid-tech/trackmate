...
export default function MyClaims() {
  ...
  // User parsing strictly inside a try-catch to avoid crashes
  const user = (() => {
    try {
  ...
  const fetchClaims = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getMyClaims(); // Using the proper API layer
      setClaims(res.data || []);
    } catch (err) {
  ...
  const handleApprove = async (claimId) => {
    ...
    try {
      ...
      showNotification(
        "error",
        "Approval Failed",
        err.response?.data?.detail || "Failed to approve the claim.",
      );
    } finally {
  ...
  const handleReject = async (claimId) => {
    ...
    try {
      ...
      showNotification(
        "error",
        "Rejection Failed",
        err.response?.data?.detail || "Failed to reject the claim.",
      );
    } finally {
      setActionLoading(null);
    }
  ...
  const handleVerifyOTP = async (claimId) => {
    ...
    setActionLoading(claimId);
    try {
      await verifyClaim(claimId, code);
      showNotification(
        "success",
        "Handover Verified!",
        "Trust scores have been updated successfully.",
      );
      fetchClaims();
    } catch (err) {
      showNotification(
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center py-32 bg-[#F9FAFB]">
  ...
  return (
    <>
      {/* 🚨 Removed min-h-screen to fix layout conflict */}
      <div className="bg-[#F9FAFB] pb-20 pt-10 w-full">
